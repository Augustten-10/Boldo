...
const Connect = () => {
  ...
  return (
    <div className="px-36 pb-20 max-sm:px-10">
      <div className="flex justify-between items-center max-sm:flex-col">
        <div className="max-w-[50%] max-sm:max-w-full">
          <img src={Connect1} alt="" />
        </div>
        <div className="w-[45%] max-sm:w-full">
          <p className="text-4xl leading-normal max-sm:text-2xl">
            We connect our customers with the best, and help them keep up-and
            stay open.
          </p>
          <div className="mt-16 flex flex-col gap-5 max-sm:mt-10">
            {lists.map((list) => (
              <div key={list.id} className="flex gap-5 items-center">
                <img src={Tick} alt="tick" />
                <p className="text-xl font-openSans font-normal max-sm:text-base">{list.info}</p>
              </div>
            ))}
          </div>
          <button className="mt-16 text-white bg-darkBlue max-sm:mt-10">Start Now</button>
        </div>
      </div>
      <div className="flex justify-between items-center mt-16 max-sm:flex-col-reverse">
        <div className="w-[45%] max-sm:w-full max-sm:mt-10">
          <p className="text-4xl leading-normal max-sm:text-2xl">
            We connect our customers with the best, and help them keep up-and
            stay open.
          </p>
